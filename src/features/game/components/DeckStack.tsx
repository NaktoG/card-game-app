    ...
    <div className="relative h-40 w-32 sm:h-48 sm:w-36" aria-hidden="true">
      {[0, 1, 2, 3].map((card) => (
        <motion.img
          ...
          alt=""
          animate={{
            rotate: isLoading ? [card * 4, card * 4 + 8, card * 4] : card * 4,
            y: isLoading ? [card * 3, card * 3 - 8, card * 3] : card * 3,
          }}
          transition={{ duration: 0.8, repeat: isLoading ? Infinity : 0, delay: card * 0.08 }}
